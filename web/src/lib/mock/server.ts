...
import "server-only";
import type {
  AnalysisStart, ApiError, ByokProvider, FindQuery, FindResult, FindStart, HistoryItem,
...
function state(): State {
  if (!g.holtMock) {
    ...
    g.holtMock = { cache, jobs: new Map(), users: new Map() };
  }
  return g.holtMock;
}
...
function user(id: string) {
  ...
  if (!u) {
    u = {
      ...
      history: [
        { repo: "psf/requests", mode: "rules", days: 7, verdict: "viable", headline: "Worth your time", created_at: new Date(Date.now() - 26 * 3_600_000).toISOString() },
        { repo: "pytorch/pytorch", mode: "rules", days: 7, verdict: "not_viable", headline: "Not worth your time", created_at: new Date(Date.now() - 50 * 3_600_000).toISOString() },
      ],
    };
    s.users.set(id, u);
  }
...
function remember(userId: string | undefined, r: Report) {
  ...
  h.unshift({ repo: r.repo, mode: r.mode, days: r.days, verdict: r.verdict, headline: r.headline, created_at: new Date().toISOString() });
  h.splice(30);
}
...
export async function startAnalysis(
  ...
  if (mode === "ai") {
    ...
    const u = user(userId);
    if (!u.me.byok) {
      if (u.me.quota.ai_limit <= 0) return err(402, "needs_key", "AI reports need a plan or your own API key.");
...
export function jobEvents(id: string, signal: AbortSignal): Response {
  const job = state().jobs.get(id);
...
export async function find(q: FindQuery): Promise<Result<FindStart>> {
  ...
  const results: FindResult[] = mockFindPool()
  ...
  return { ok: true, data: { status: "done", results } };
}
